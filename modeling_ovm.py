...
class MistralForCausalLM(MistralPreTrainedModel):
    ...
    def forward(
        ...
        labels: Optional[torch.FloatTensor] = None,  # Change the type to torch.FloatTensor
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        ...
        if labels is not None:
            ...
            shift_labels = labels[..., 1:].contiguous()
            # Flatten the tokens
            shift_logits = shift_logits.view(-1)  # Flatten the logits
            shift_labels = shift_labels.view(-1)  # Flatten the labels
            # Ensure tensors are on the same device
            shift_labels = shift_labels.to(shift_logits.device)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                # loss_fct = torch.nn.CrossEntropyLoss(weight=torch.tensor([neg_weights, pos_weights], device=self.device))
